...
/*   Updated: 2024/06/17 01:06:01 by aalatzas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void give_fork_back(t_philo *ph)
{
	if (death_check(ph->env) != true)
	{
		...
		pthread_mutex_unlock(&ph->env->l_fork[(ph->index % ph->env->philo_nbr) + 1]);
	}
...
void try_take_fork(t_philo *ph)
{
    ...
    else
    {
        if (ph->index % 2 == 0)
        {
            pthread_mutex_lock(&ph->env->l_fork[ph->index]);
            print_msg(ph->env, ph->index, "has taken a fork", E_TAKE_FORK);
            pthread_mutex_lock(&ph->env->l_fork[(ph->index % ph->env->philo_nbr) + 1]);
            print_msg(ph->env, ph->index, "has taken a fork", E_TAKE_FORK);
        }
        else
        {
